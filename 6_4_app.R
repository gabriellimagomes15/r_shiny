...
#### 6.4. Inserindo InfoBox e ValueBox (estático e dinâmico)
...
corpo_pagina  <- dashboardBody(
  fluidRow(
    # valueBox com qtd de registros da base inicial ('estatico')
    valueBox(subtitle =  "Registros",value = nrow(dados), 
             icon = icon("database")),
    
    # infobox com media ('estatico')
               ...
               column(width = 12,
                  box(width = '100%',
                      ...
                      awesomeCheckboxGroup(inputId = "select_UF",label =  "Estado:",inline = TRUE,
                                           choices = c('TODOS',sort(unique(dados$UF))),selected = 'TODOS')
                  )
                 ),
               column(width = 6,
                      box(width = '60%',
                      ## calendário para selecionar per??odo
...
server = function(input, output, session) {
  ...
  output$approvalBox <- renderValueBox({
    valueBox(value = length(unique(dados_selecionados()$UF)), 
             subtitle = "Estados Selecionados", icon = icon("map-marker"),
